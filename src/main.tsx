...
import { TrainerProfile, createDefaultTrainerProfile } from './types/trainer.js';
...
const educationalPartnershipService = EducationalPartnershipService.getInstance();
...
Devvit.addCustomPostType({
  ...
  render: (context) => {
    ...
    function generateWildAnimal() {
      const wildAnimals = [
        { 
          ...
          baseStats: { health: 85, attack: 75, defense: 60, speed: 70, intelligence: 50, stamina: 65 }
        },
        { 
          name: 'Wolf', 
          emoji: '🐺', 
          ...
          baseStats: { health: 70, attack: 70, defense: 55, speed: 80, intelligence: 60, stamina: 70 }
        },
        { 
          name: 'Bear', 
          emoji: '🐻', 
          ...
          baseStats: { health: 100, attack: 80, defense: 75, speed: 45, intelligence: 55, stamina: 80 }
        },
        { 
          name: 'Eagle', 
          emoji: '🦅', 
          type: ['mountain'], 
          level: Math.floor(Math.random() * 3) + 4, // Level 4-6
          baseStats: { health: 65, attack: 65, defense: 50, speed: 95, intelligence: 75, stamina: 60 }
        },
        { 
          name: 'Shark', 
          emoji: '🦈', 
          ...
          baseStats: { health: 90, attack: 85, defense: 70, speed: 75, intelligence: 45, stamina: 75 }
        }
      ];
      ...
      const moves = generateMovesForAnimal(randomAnimal.name, randomAnimal.type, level);
    ...
    function generateMovesForAnimal(name: string, types: string[], level: number) {
      const movesByType: Record<string, any[]> = {
        grassland: [
          { id: 'bite', name: 'Bite', description: 'A powerful bite attack.', type: 'grassland', power: 60, accuracy: 100, energyCost: 10 },
          { id: 'charge', name: 'Charge', description: 'Rush forward with tremendous force.', type: 'grassland', power: 90, accuracy: 85, energyCost: 20 },
          { id: 'roar', name: 'Roar', description: 'Let out a terrifying roar.', type: 'grassland', power: 0, accuracy: 100, energyCost: 8 }
        ],
        forest: [
          { id: 'claw', name: 'Claw', description: 'Slash with sharp claws.', type: 'forest', power: 40, accuracy: 100, energyCost: 8 },
          { id: 'bite', name: 'Bite', description: 'A powerful bite attack.', type: 'forest', power: 60, accuracy: 100, energyCost: 10 },
          { id: 'howl', name: 'Howl', description: 'Howl to boost attack power.', type: 'forest', power: 0, accuracy: 100, energyCost: 8 }
        ],
        mountain: [
          { id: 'wing_attack', name: 'Wing Attack', description: 'Strike with powerful wings.', type: 'mountain', power: 60, accuracy: 100, energyCost: 12 },
          { id: 'air_slash', name: 'Air Slash', description: 'Cut through the air with razor-sharp wind.', type: 'mountain', power: 75, accuracy: 95, energyCost: 18 },
          { id: 'dive_bomb', name: 'Dive Bomb', description: 'Dive from great height.', type: 'mountain', power: 100, accuracy: 95, energyCost: 20 }
        ],
        ocean: [
          { id: 'water_gun', name: 'Water Gun', description: 'Spray a powerful jet of water.', type: 'ocean', power: 40, accuracy: 100, energyCost: 10 },
          { id: 'bite', name: 'Bite', description: 'A powerful bite attack.', type: 'ocean', power: 60, accuracy: 100, energyCost: 10 },
          { id: 'tidal_wave', name: 'Tidal Wave', description: 'Summon a massive wave.', type: 'ocean', power: 110, accuracy: 80, energyCost: 25 }
        ]
      };
        ...
        try {
          switch (messageType) {
            case 'webViewReady':
              ...
              const profileData = username ? await context.redis.get(`trainer_profile_${username}`) : null;
              if (profileData) {
                trainerProfile = JSON.parse(profileData);
              } else if (username) {
                trainerProfile = createDefaultTrainerProfile(username, `trainer_${username}_${Date.now()}`);
                await context.redis.set(`trainer_profile_${username}`, JSON.stringify(trainerProfile));
              } else {
                trainerProfile = createDefaultTrainerProfile('anonymous', 'trainer_anon');
              }

              // Process daily rewards
              const sessionId = `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
              const dailyReward = username ? await economyService.processDailyRewardsWithTracking(trainerProfile, sessionId) : null;
              ...
              const availableHabitats = habitatService.getAvailableHabitats(trainerProfile);
              const dailyExpeditions = await expeditionService.generateDailyExpeditions(trainerProfile);
              ...
              webView.postMessage({
                ...
                data: {
                  ...
                  trainerProfile: {
                    ...
                    premiumTier: economyService.getPremiumManager().getPremiumTier(trainerProfile)
                  },
            ...
            case 'battleAction': {
              ...
              if (action === 'attack') {
                ...
                webView.postMessage({
                  ...
                  data: {
                    type: 'battleMessage',
                    message: playerWins ? 'Wild animal fainted!' : 'Your animal fainted!'
                  }
                });
            ...
            case 'purchaseItem': {
              if (!username || !message.data?.itemId) {
                webView.postMessage({ type: 'purchaseResult', data: { success: false, error: 'Invalid request' } });
                break;
              }

              const profileData = await context.redis.get(`trainer_profile_${username}`);
              if (!profileData) {
                webView.postMessage({ type: 'purchaseResult', data: { success: false, error: 'Profile not found' } });
                break;
              }
            ...
            case 'purchasePremium': {
              if (!username || !message.data?.tier) {
                webView.postMessage({ type: 'premiumResult', data: { success: false, error: 'Invalid request' } });
                break;
              }

              const profileData = await context.redis.get(`trainer_profile_${username}`);
              if (!profileData) {
                webView.postMessage({ type: 'premiumResult', data: { success: false, error: 'Profile not found' } });
                break;
              }
            ...
            case 'purchaseCosmetic': {
              if (!username || !message.data?.cosmeticId) {
                webView.postMessage({ type: 'cosmeticResult', data: { success: false, error: 'Invalid request' } });
                break;
              }

              const profileData = await context.redis.get(`trainer_profile_${username}`);
              if (!profileData) {
                webView.postMessage({ type: 'cosmeticResult', data: { success: false, error: 'Profile not found' } });
                break;
              }
            ...
            case 'getCosmeticStore': {
              const store = economyService.getPremiumManager().getCosmeticStore();
              webView.postMessage({
                type: 'cosmeticStore',
                data: { items: store }
              });
            ...
            case 'useItem': {
              if (!username || !message.data?.itemId) {
                webView.postMessage({ type: 'useItemResult', data: { success: false, error: 'Invalid request' } });
                break;
              }

              const profileData = await context.redis.get(`trainer_profile_${username}`);
              if (!profileData) {
                webView.postMessage({ type: 'useItemResult', data: { success: false, error: 'Profile not found' } });
                break;
              }
            ...
            case 'getWorldMap': {
              if (!username) {
                webView.postMessage({ type: 'worldMapError', data: { error: 'User not authenticated' } });
                break;
              }

              const profileData = await context.redis.get(`trainer_profile_${username}`);
              if (!profileData) {
                webView.postMessage({ type: 'worldMapError', data: { error: 'Profile not found' } });
                break;
              }
            ...
            case 'getHabitatInfo': {
              ...
              if (!habitatId) {
                webView.postMessage({ type: 'habitatInfoError', data: { error: 'Habitat ID required' } });
                break;
              }

              const habitat = habitatService.getHabitat(habitatId);
              if (!habitat) {
                webView.postMessage({ type: 'habitatInfoError', data: { error: 'Habitat not found' } });
                break;
              }
            ...
            case 'exploreHabitat': {
              if (!username || !message.data?.habitatId) {
                webView.postMessage({ type: 'explorationError', data: { error: 'Invalid request' } });
                break;
              }

              const profileData = await context.redis.get(`trainer_profile_${username}`);
              if (!profileData) {
                webView.postMessage({ type: 'explorationError', data: { error: 'Profile not found' } });
                break;
              }
              ...
              try {
                ...
                const habitat = habitatService.getHabitat(message.data.habitatId);
                if (habitat) {
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Exploration failed' }
                });
              }
              break;
            }
            case 'getDailyExpeditions': {
              if (!username) {
                webView.postMessage({ type: 'expeditionsError', data: { error: 'User not authenticated' } });
                break;
              }

              const profileData = await context.redis.get(`trainer_profile_${username}`);
              if (!profileData) {
                webView.postMessage({ type: 'expeditionsError', data: { error: 'Profile not found' } });
                break;
              }

              try {
                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const expeditions = await expeditionService.generateDailyExpeditions(trainerProfile);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get expeditions' }
                });
              }
              break;
            }
            case 'startExpedition': {
              if (!username || !message.data?.expeditionId) {
                webView.postMessage({ type: 'expeditionError', data: { error: 'Invalid request' } });
                break;
              }

              try {
                const result = await expeditionService.startExpedition(message.data.expeditionId, username);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Expedition failed' }
                });
              }
              break;
            }
            case 'getWeatherForecast': {
              const habitatId = message.data?.habitatId;
              if (!habitatId) {
                webView.postMessage({ type: 'weatherError', data: { error: 'Habitat ID required' } });
                break;
              }

              try {
                const forecast = weatherSystem.getWeatherForecast(habitatId);
                const currentWeather = weatherSystem.getCurrentWeather(habitatId);
                ...
                webView.postMessage({
                  type: 'weatherError',
                  data: { error: error instanceof Error ? error.message : 'Weather data unavailable' }
                });
              }
              break;
            }
            case 'discoverHiddenArea': {
              if (!username || !message.data?.areaId) {
                webView.postMessage({ type: 'discoveryError', data: { error: 'Invalid request' } });
                break;
              }

              try {
                const result = await worldMapService.discoverHiddenArea(message.data.areaId, username);
                ...
                webView.postMessage({
                  type: 'discoveryError',
                  data: { error: error instanceof Error ? error.message : 'Discovery failed' }
                });
              }
              break;
            }
            case 'getEducationalPartners': {
              ...
              const organizations = educationalPartnershipService.getActiveOrganizations();
              webView.postMessage({
            ...
            case 'getSponsoredContent': {
              if (!username) {
                webView.postMessage({ type: 'sponsoredContentError', data: { error: 'User not authenticated' } });
                break;
              }

              const profileData = await context.redis.get(`trainer_profile_${username}`);
              if (!profileData) {
                webView.postMessage({ type: 'sponsoredContentError', data: { error: 'Profile not found' } });
                break;
              }

              const trainerProfile: TrainerProfile = JSON.parse(profileData);
              const sponsoredContent = educationalPartnershipService.getSponsoredContent(trainerProfile);
            ...
            case 'completeSponsoredContent': {
              if (!username || !message.data?.contentId) {
                webView.postMessage({ type: 'contentCompletionError', data: { error: 'Invalid request' } });
                break;
              }

              const result = await educationalPartnershipService.completeSponsoredContent(
                username,
            ...
            case 'getConservationImpact': {
              if (!username) {
                webView.postMessage({ type: 'conservationImpactError', data: { error: 'User not authenticated' } });
                break;
              }

              const impact = educationalPartnershipService.getConservationImpact(username);
              const totalImpact = educationalPartnershipService.getTotalConservationImpact();
            ...
            case 'submitAMAQuestion': {
              if (!username || !message.data?.amaId || !message.data?.question) {
                webView.postMessage({ type: 'amaQuestionError', data: { error: 'Invalid request' } });
                break;
              }
            ...
            case 'getCitizenScienceProjects': {
              const projects = educationalPartnershipService.getActiveCitizenScienceProjects();
              webView.postMessage({
            ...
            case 'submitDataContribution': {
              if (!username || !message.data?.projectId || !message.data?.dataType || !message.data?.data) {
                webView.postMessage({ type: 'dataContributionError', data: { error: 'Invalid request' } });
                break;
              }

              const success = educationalPartnershipService.submitDataContribution(
            ...
            case 'getTrainerContributions': {
              if (!username) {
                webView.postMessage({ type: 'contributionsError', data: { error: 'User not authenticated' } });
                break;
              }

              const contributions = educationalPartnershipService.getTrainerContributions(username);
              webView.postMessage({
            ...
            case 'createBattleReplay': {
              if (!username || !message.data?.battleId) {
                webView.postMessage({ type: 'replayError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'replayError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const replay = await contentCreationService.createBattleReplay(message.data.battleId, trainerProfile.trainerId);
                const redditPost = await redditPostGenerator.generateBattleReplayPost(replay, trainerProfile);
                ...
                webView.postMessage({
                  type: 'replayError',
                  data: { error: error instanceof Error ? error.message : 'Failed to create replay' }
                });
              }
              break;
            }
            case 'createCollectionShowcase': {
              if (!username || !message.data?.category || !message.data?.animals) {
                webView.postMessage({ type: 'showcaseError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'showcaseError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const showcase = await contentCreationService.createCollectionShowcase(
                  trainerProfile.trainerId,
                  message.data.category,
                  message.data.animals
                );
                const redditPost = await redditPostGenerator.generateCollectionShowcasePost(showcase, trainerProfile);
                ...
                webView.postMessage({
                  type: 'showcaseError',
                  data: { error: error instanceof Error ? error.message : 'Failed to create showcase' }
                });
              }
              break;
            }
            case 'createStrategyGuide': {
              if (!username || !message.data?.content) {
                webView.postMessage({ type: 'strategyGuideError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'strategyGuideError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const guide = await contentCreationService.generateStrategyGuide(trainerProfile.trainerId, message.data.content);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to create strategy guide' }
                });
              }
              break;
            }
            case 'submitFanArt': {
              if (!username || !message.data?.artwork) {
                webView.postMessage({ type: 'fanArtError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'fanArtError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const fanArt = await contentCreationService.submitFanArt(trainerProfile.trainerId, message.data.artwork);
                ...
                webView.postMessage({
                  type: 'fanArtError',
                  data: { error: error instanceof Error ? error.message : 'Failed to submit fan art' }
                });
              }
              break;
            }
            case 'getStrategyGuides': {
              ...
              const featuredContent = contentCreationService.getFeaturedContent();
            ...
            case 'getCommunityContribution': {
              if (!username) {
                webView.postMessage({ type: 'contributionError', data: { error: 'User not authenticated' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'contributionError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const contribution = await communityRewardsService.calculateCommunityContribution(trainerProfile.trainerId);
                const topContributors = communityRewardsService.getTopContributors(10);
                const communityStats = communityRewardsService.getCommunityStats();
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get contribution data' }
                });
              }
              break;
            }
            case 'generateInfographic': {
              if (!message.data?.infographicData) {
                webView.postMessage({ type: 'infographicError', data: { error: 'Invalid request' } });
                break;
              }

              try {
                const infographicUrl = await redditPostGenerator.generateInfographic(message.data.infographicData);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to generate infographic' }
                });
              }
              break;
            }
            case 'createTournament': {
              if (!username || !message.data?.config) {
                webView.postMessage({ type: 'tournamentError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'tournamentError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const config = { ...message.data.config, createdBy: trainerProfile.trainerId };
                const tournament = await tournamentService.createTournament(config);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to create tournament' }
                });
              }
              break;
            }
            case 'registerForTournament': {
              if (!username || !message.data?.tournamentId) {
                webView.postMessage({ type: 'registrationError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'registrationError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const success = await tournamentService.registerParticipant(message.data.tournamentId, trainerProfile.trainerId);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to register for tournament' }
                });
              }
              break;
            }
            ...
            case 'getTournamentStandings': {
              if (!message.data?.tournamentId) {
                webView.postMessage({ type: 'standingsError', data: { error: 'Tournament ID required' } });
                break;
              }

              try {
                const standings = await tournamentService.getTournamentStandings(message.data.tournamentId);
                ...
                webView.postMessage({
                  type: 'standingsError',
                  data: { error: error instanceof Error ? error.message : 'Failed to get standings' }
                });
              }
              break;
            }
            case 'getWeeklyTournament': {
              try {
                const weeklyTournament = await tournamentService.createWeeklyTournament();
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get weekly tournament' }
                });
              }
              break;
            }
            case 'getPlayerRank': {
              if (!username) {
                webView.postMessage({ type: 'rankError', data: { error: 'User not authenticated' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'rankError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const playerRank = await rankingService.calculatePlayerRank(trainerProfile.trainerId);
                ...
                webView.postMessage({
                  type: 'rankError',
                  data: { error: error instanceof Error ? error.message : 'Failed to get player rank' }
                });
              }
              break;
            }
            case 'getRankingLeaderboard': {
              ...
              try {
                const leaderboard = await rankingService.getLeaderboard(category, timeframe);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get leaderboard' }
                });
              }
              break;
            }
            case 'findMatchmakingOpponent': {
              if (!username) {
                webView.postMessage({ type: 'matchmakingError', data: { error: 'User not authenticated' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'matchmakingError', data: { error: 'Profile not found' } });
                  break;
                }
                ...
                const opponentId = await rankingService.findMatchmakingOpponent(trainerProfile.trainerId, preferences);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to find opponent' }
                });
              }
              break;
            }
            case 'createSpectatorSession': {
              if (!message.data?.matchId) {
                webView.postMessage({ type: 'spectatorError', data: { error: 'Match ID required' } });
                break;
              }

              try {
                const session = await spectatorService.createSpectatorSession(message.data.matchId);
                ...
                webView.postMessage({
                  type: 'spectatorError',
                  data: { error: error instanceof Error ? error.message : 'Failed to create spectator session' }
                });
              }
              break;
            }
            case 'joinSpectatorSession': {
              if (!username || !message.data?.sessionId) {
                webView.postMessage({ type: 'spectatorError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'spectatorError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const success = await spectatorService.joinSpectatorSession(message.data.sessionId, trainerProfile.trainerId);
                ...
                webView.postMessage({
                  type: 'spectatorError',
                  data: { error: error instanceof Error ? error.message : 'Failed to join spectator session' }
                });
              }
              break;
            }
            case 'getActiveSpectatorSessions': {
              const activeSessions = spectatorService.getActiveSpectatorSessions();
            ...
            case 'sendSpectatorChat': {
              if (!username || !message.data?.sessionId || !message.data?.message) {
                webView.postMessage({ type: 'chatError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'chatError', data: { error: 'Profile not found' } });
                  break;
                }
                ...
                webView.postMessage({
                  type: 'chatError',
                  data: { error: error instanceof Error ? error.message : 'Failed to send chat message' }
                });
              }
              break;
            }
            case 'updateMatchAfterBattle': {
              if (!username || !message.data?.opponentId || !message.data?.result) {
              ...
              try {
                ...
                // Get updated rank
                const updatedRank = await rankingService.calculatePlayerRank(trainerProfile.trainerId);
            ...
            case 'createGuild': {
              if (!username || !message.data?.config) {
                webView.postMessage({ type: 'guildError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'guildError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const guild = await guildService.createGuild(trainerProfile.trainerId, message.data.config);
                ...
                webView.postMessage({
                  type: 'guildError',
                  data: { error: error instanceof Error ? error.message : 'Failed to create guild' }
                });
              }
              break;
            }
            case 'joinGuild': {
              if (!username || !message.data?.guildId) {
                webView.postMessage({ type: 'guildJoinError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'guildJoinError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const success = await guildService.joinGuild(message.data.guildId, trainerProfile.trainerId);
                ...
                webView.postMessage({
                  type: 'guildJoinError',
                  data: { error: error instanceof Error ? error.message : 'Failed to join guild' }
                });
              }
              break;
            }
            ...
            case 'getPlayerGuild': {
              if (!username) {
                webView.postMessage({ type: 'playerGuildError', data: { error: 'User not authenticated' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'playerGuildError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const guild = guildService.getGuildByTrainer(trainerProfile.trainerId);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get player guild' }
                });
              }
              break;
            }
            case 'startGuildRaid': {
              if (!username || !message.data?.guildId || !message.data?.raidConfig) {
                webView.postMessage({ type: 'raidError', data: { error: 'Invalid request' } });
                break;
              }

              try {
                const raid = await guildService.startGuildRaid(message.data.guildId, message.data.raidConfig);
                ...
                webView.postMessage({
                  type: 'raidError',
                  data: { error: error instanceof Error ? error.message : 'Failed to start guild raid' }
                });
              }
              break;
            }
            case 'createMentorProfile': {
              if (!username || !message.data?.expertise) {
                webView.postMessage({ type: 'mentorError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'mentorError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const mentorProfile = await mentorshipService.createMentorProfile(trainerProfile.trainerId, message.data.expertise);
                ...
                webView.postMessage({
                  type: 'mentorError',
                  data: { error: error instanceof Error ? error.message : 'Failed to create mentor profile' }
                });
              }
              break;
            }
            case 'requestMentorship': {
              if (!username || !message.data?.mentorId || !message.data?.focus) {
                webView.postMessage({ type: 'mentorshipRequestError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'mentorshipRequestError', data: { error: 'Profile not found' } });
                  break;
                }
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to request mentorship' }
                });
              }
              break;
            }
            case 'getMentorRecommendations': {
              if (!username) {
                webView.postMessage({ type: 'mentorRecommendationsError', data: { error: 'User not authenticated' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'mentorRecommendationsError', data: { error: 'Profile not found' } });
                  break;
                }

                const trainerProfile: TrainerProfile = JSON.parse(profileData);
                const recommendations = await mentorshipService.getMentorRecommendations(trainerProfile.trainerId);
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get mentor recommendations' }
                });
              }
              break;
            }
            ...
            case 'createCommunityVote': {
              if (!username || !message.data?.proposal) {
                webView.postMessage({ type: 'voteError', data: { error: 'Invalid request' } });
                break;
              }

              try {
                const vote = await mentorshipService.createCommunityVote(message.data.proposal);
                ...
                webView.postMessage({
                  type: 'voteError',
                  data: { error: error instanceof Error ? error.message : 'Failed to create community vote' }
                });
              }
              break;
            }
            case 'castCommunityVote': {
              if (!username || !message.data?.voteId || !message.data?.choice) {
                webView.postMessage({ type: 'castVoteError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                const profileData = await context.redis.get(`trainer_profile_${username}`);
                if (!profileData) {
                  webView.postMessage({ type: 'castVoteError', data: { error: 'Profile not found' } });
                  break;
                }
                ...
                webView.postMessage({
                  type: 'castVoteError',
                  data: { error: error instanceof Error ? error.message : 'Failed to cast vote' }
                });
              }
              break;
            }
            ...
            case 'createSupportTicket': {
              if (!username || !message.data?.subject || !message.data?.description) {
                webView.postMessage({ type: 'supportTicketError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to create ticket' }
                });
              }
              break;
            }
            case 'getSupportTickets': {
              if (!username) {
                webView.postMessage({ type: 'supportTicketsError', data: { error: 'User not authenticated' } });
                break;
              }
              ...
              try {
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get tickets' }
                });
              }
              break;
            }
            case 'getSupportTicket': {
              if (!message.data?.ticketId) {
                webView.postMessage({ type: 'supportTicketError', data: { error: 'Ticket ID required' } });
                break;
              }
              ...
              try {
                ...
                if (!ticket) {
                  webView.postMessage({ type: 'supportTicketError', data: { error: 'Ticket not found' } });
                  break;
                }
                ...
                webView.postMessage({
                  type: 'supportTicketError',
                  data: { error: error instanceof Error ? error.message : 'Failed to get ticket' }
                });
              }
              break;
            }
            case 'addSupportResponse': {
              if (!username || !message.data?.ticketId || !message.data?.content) {
                webView.postMessage({ type: 'supportResponseError', data: { error: 'Invalid request' } });
                break;
              }

              try {
                const success = await supportSystem.addResponse(message.data.ticketId, {
                  authorId: username,
                  authorType: 'user',
                ...
                webView.postMessage({
                  type: 'supportResponseError',
                  data: { error: error instanceof Error ? error.message : 'Failed to add response' }
                });
              }
              break;
            }
            case 'searchFAQs': {
              ...
              try {
                ...
                webView.postMessage({
                  type: 'faqError',
                  data: { error: error instanceof Error ? error.message : 'Failed to search FAQs' }
                });
              }
              break;
            }
            case 'rateFAQ': {
              if (!message.data?.faqId || message.data?.helpful === undefined) {
                webView.postMessage({ type: 'faqRatingError', data: { error: 'Invalid request' } });
                break;
              }

              try {
                const success = supportSystem.rateFAQ(message.data.faqId, message.data.helpful);
                webView.postMessage({
                  type: 'faqRated',
                  data: { success }
                });
                ...
                webView.postMessage({
                  type: 'faqRatingError',
                  data: { error: error instanceof Error ? error.message : 'Failed to rate FAQ' }
                });
              }
              break;
            }
            case 'submitUserReport': {
              if (!username || !message.data?.targetUserId || !message.data?.category) {
                webView.postMessage({ type: 'reportError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                ...
                webView.postMessage({
                  type: 'reportError',
                  data: { error: error instanceof Error ? error.message : 'Failed to submit report' }
                });
              }
              break;
            }
            case 'moderateContent': {
              if (!message.data?.content || !message.data?.userId) {
                webView.postMessage({ type: 'moderationError', data: { error: 'Invalid request' } });
                break;
              }
              ...
              try {
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to moderate content' }
                });
              }
              break;
            }
            case 'getUserRestrictions': {
              if (!message.data?.userId) {
                webView.postMessage({ type: 'restrictionsError', data: { error: 'User ID required' } });
                break;
              }

              try {
                const restrictions = moderationSystem.getUserRestrictions(message.data.userId);
                webView.postMessage({
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get restrictions' }
                });
              }
              break;
            }
            case 'getSupportStats': {
              ...
              if (!username) {
                webView.postMessage({ type: 'supportStatsError', data: { error: 'Unauthorized' } });
                break;
              }
              ...
              try {
                ...
                webView.postMessage({
                  ...
                  data: { error: error instanceof Error ? error.message : 'Failed to get stats' }
                });
              }
              break;
            }
